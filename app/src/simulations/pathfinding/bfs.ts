...
export function bfsStep(state: PathfindingState): PathfindingState {
  if (state.done || state.frontier.length === 0) {
    return { ...state, done: true, explanation: '탐색이 완료되었습니다. 경로를 찾지 못했습니다.' }
  }

  // 불변 복사: shift()로 원본 배열을 직접 변경하지 않도록 먼저 복사
  const newFrontier = [...state.frontier]
  const current = newFrontier.shift()!
  ...
  if (current.row === state.goal.row && current.col === state.goal.col) {
    ...
    return {
      ...
      found: true,
      explanation: `목표 도달! ${path.length - 1}칸 거리의 최단 경로를 찾았습니다. BFS는 가중치 없는 그래프에서 항상 최단경로를 보장합니다.`,
    }
  }
  ...
  return {
    ...
    parent: newParent,
    step: state.step + 1,
    explanation: `BFS: 큐에서 (${current.row}, ${current.col}) 꺼냄. 방문: ${Object.keys(newVisited).length}칸. 대기: ${newFrontier.length}칸.`,
  }
}
